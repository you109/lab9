...
library(ggplot2)
hist_TPA <- ggplot(sum_u2, aes(x = TPA)) +
  geom_histogram(binwidth = 50, fill = "lightpink", color = "black") +
  labs(title = "Distribution of TPA (Trees per Acre)", x = "TPA", y = "Frequency")
print(hist_TPA)


hist_BA <- ggplot(sum_u2, aes(x = BA)) +
  geom_histogram(binwidth = 10, fill = "blue", color = "black") +
  labs(title = "Distribution of BA (Basal Area)", x = "BA (sq ft/acre)", y = "Frequency")
print(hist_BA)

hist_biomass <- ggplot(sum_u2, aes(x = biomass)) +
  geom_histogram(binwidth = 20, fill = "lightblue", color = "black") +
  labs(title = "Distribution of Biomass", x = "Biomass (tons/acre)", y = "Frequency")

print(hist_biomass)

table(sum_u2$Dom_species)
